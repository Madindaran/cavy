...
export default function(spec) {
  ...
  spec.describe('Tapping on an employee', function() {
    ...
    spec.it('clicking call button ',async ()=>{
      await spec.fillIn('SearchBar.TextInput', 'Amy');
      await spec.press('EmployeeList.AmyTaylor');
      await spec.pause(1000);
      await spec.containsText('EmployeeList.CEO', 'CEO');
      await spec.press('ActionBar.CallButton');
      await spec.press('ActionBar.BackButton')
    })
    spec.it('clicking message button ',async()=>{
      await spec.fillIn('SearchBar.TextInput', 'Amy');
      await spec.press('EmployeeList.AmyTaylor');
      await spec.pause(1000);
      await spec.containsText('EmployeeList.CEO', 'CEO');
      await spec.press('ActionBar.MessageButton');
    })
  });
